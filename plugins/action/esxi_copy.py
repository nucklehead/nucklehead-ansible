...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        # look up the files and use the first one we find as src
            ...
            result.update(self._execute_module(module_name='esxi_copy', module_args=self._task.args, task_vars=task_vars, delete_remote_tmp=False))
            return result
        ...
        for source_full, source_rel in source_files:
            ...
            if local_checksum != remote_checksum:
                ...
                module_return = self._execute_module(module_name='esxi_copy', module_args=new_module_args, task_vars=task_vars, delete_remote_tmp=delete_remote_tmp)
                module_executed = True

            else:
                if source_rel and dest.endswith(os.sep):
                    module_return["dest"] = os.path.join(dest, source_rel)
                    module_executed = True
